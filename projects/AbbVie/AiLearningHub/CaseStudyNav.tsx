...
const SECTIONS = [
  ...
  { id: "deliverables", label: "The Brief" },
  { id: "approaches", label: "Approaches" },
  { id: "recommendation", label: "The Reframe" },
...
export function CaseStudyNav({ accent }: { accent: string }) {
  ...
  useEffect(() => {
    const content = document.getElementById("alh-content");
    if (!content) return;
    // Visible only while the content region still crosses the viewport's
    // vertical middle, where this centred rail sits — so it fades out before
    // the section below scrolls up to meet it.
        ...
        return (
          <a key={s.id} href={`#${s.id}`} className="group flex items-center gap-3">
            <span
              ...
              style={{ width: isActive ? 22 : 8, height: 2, background: isActive ? accent : "rgba(38,38,42,0.3)" }}
            />
            <span
